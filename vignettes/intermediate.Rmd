...
- mediator_LR: likelihood ratio of driver effect on mediators
- mediation_scan: scan over mediators by position for mediation of driver on target
- mediation_test: causal model selection test for mediation
- mediation_effect: effect of driver on target adjusted by mediator 
- mediation_triad: scatterplot of target on mediator by driver

In this example, there is only one driver and many possible mediators.
The driver is known to affect the target. The question is whether one or more mediators can explain away the driver effect.

In other settings, there may be different drivers for different mediators,
and those drivers may be different from the putative target driver. The routine `mediation_test` is set up to allow for this situation, using information from the `annotation` object. Basically, a mediator may have a position, such as the location of the gene associated with mRNA as in `Tmem68`, and may have a driver position. For more information, see
package 'qtl2mediate'.
...
This version only contains data for chr 4 and 13, and has had the following changes to elements:

- `covar`: first two columns renamed to `Intercept` and `SexM`;
- `driver`: renames from the original `qtl.geno`.

See help for `Tmem68` for more details.
...
First find mediators that have significant LR. This will be used to filter traits to a set of potential mediators, and provide annotaton for those mediators.
...
Mediation scan regresses the target on driver and mediator, and indexes them by the position of the mediator. A strong mediator would drop the LR down toward 0.
...
Causal model selection tests are performed to see which of `causal`, `reactive`, or `independent` models best fits the data, using intersection-union formal testing (see Chaibub Neto et al. paper).
A fourth model, `undecided`, is also examined. See details in the last section.
...
This routine looks at the effects of the driver on the mediator, on the target, and on the target adjusted by the mediator.
...
Mediation triads consider visually how the target and mediator are correlated. The target is regressed on the driver, directly or in reduced form (via `sdp`), and the lines are overlaid on the scatterplot of points identified by their driver value. In this situation, the driver represents the proportion of 8 genotype alleles, and it is possible to guess the allele pair per individual from this information. The `sdp` (SNP distribution pattern) reduction is a coding of the most common allele breakdown, which in this case is `sdp = 2` corresponding to the second allele against the others, or `B:ACDEFGH`.

In this example, with determined driver, the `sdp` is inferred from the target. When there are multiple drivers, the goal is to find drivers with the same `sdp` pattern for target and mediator. That is, the earlier analysis corresponded to using the driver as is, which for `Tmem68` corresponds to allele frequency per sample. With 8 alleles, there are 7 d.f. in the model, making it difficult to detect subtle effects. By reducing to a good choice of `sdp`, the driver becomes only 3 values -- 0, 1, 2 or `ref`erence (0 `B`s), `het`erozygous (`B` and something else), and `alt`ernative (2 `B`s) -- and hence 2 d.f., yielding a more powerful model for mediation.
