...
The data and original 'intermediate' code come from <https://github.com/churchill-lab/intermediate>.

```{r}
data(Tmem68, package = "Tmem68")
```

Note that the `target` and one of the mediators are identical
...
```{r}
ggplot2::autoplot(subset(med_lod, chr = c("4","13")), size = 3)
```
...
Or use `ggplot2::autoplot`, and maybe focus on a few groups and add a vertical line.

```{r}
ggplot2::autoplot(subset(med_scan, c("4","13")), size = 3) +
  ggplot2::geom_vline(xintercept = annotation[m,"pos"], linetype = "dashed")
...
med_test <- mediation_test(target = target,
                      ...
                      covar_med = covar,
                      method = "double-lod-diff")
(sum_med <- dplyr::arrange(summary(med_test), pvalue))
```
...
ggplot2::ggplot(dplyr::filter(dat, pvalue <= 0.05)) +
