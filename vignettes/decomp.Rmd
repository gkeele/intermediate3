...
The purpose of this document is to develop math to modify R/qtl2 so that the kinship object can be decomposed once and used multiple times. The challenge is that the current setup decomposes kinship on each reduced set of data, chosen to eliminate for instance individuals with missing data for a phenotype, or to restrict to a subset of individuals to improve computation.
...
Thus, to solve the sub-problem, we use the SVD for all the data, divide the eigen matrix $U$ horizontally, and do a QR decomposition on $U_1^{\text{T}}$ to obtain $G_1^{-\text{T}}$ for premultiplying $y_1$ and $X_1$. Some care will need to be taken about the condition number of matrices, but that is already incorporated into `R/qtl2` routines. 
...
```{r}
datapath <- "~/Documents/Research/attie_alan/DO/data/DerivedData/wave4"
```
...
```{r}
system.time(Ke <- qtl2::decomp_kinship(kinship))
```
...
system.time(Ke1 <- qtl2::decomp_kinship(K11))
```
...
Thus, to solve the sub-problem, we use the SVD for all the data, divide the eigen matrix $U$ horizontally, and do a QR decomposition on $U_1^{\text{T}}$ to obtian $G_1^{-\text{T}}$ for premultiplying $y_1$ and $X_1$. Some care will need to be taken about the condition number of matrices, but that is already incorporated into `R/qtl2` routines. 

The `R/qtl2` code uses `K` for the kinship matrix and `hsq` for heritability ($\sigma_g^2/(\sigma_g^2+\sigma^2)$). The decomposed eigen object is `Ke`. Decomposition is done in 
`qtl2::decomp_kinship()`. Three un-exported (and one exported) routines call this, `qtl2:::scan1_pg()`, `qtl2:::scan1coef_pg()`, `qtl2:::scan1blup_pg()` and `qtl2::est_herit()`. 

They calculate heritability `hsq` with `qtl2:::calc_hsq_clean()` using internal `by_chr_func()`, which is unexported and found in file `scan1_pg.R`. This is called with the eigen decomposition stored in the `kinship` object, as `kinship$vectors` and `kinship$values`. For `scan1()`, calculations are done in `qtl2:::scan1_pg_clean()` and flavors of `scan_pg_onechr()`, which are written in Cpp.

It should be possible to have one routine that calls `qtl2::decomp_kinship()` and `qtl2:::calc_hsq_clean()` if needed, used for the four instances identified above. We would also want to either use these or something similar as exported routine to precompute SVD once for later use.
